...
coverage_by_age_aggregated<-function(country, province, year){
  ...
  vac_year<-save_object[[1]][which(grepl(country, row.names(save_object[[1]]))), year-1950, ]
  pop_year<-save_object[[2]][which(grepl(country, row.names(save_object[[2]]))), year-1950, ]
